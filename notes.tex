...
\begin{proof}
    $F$ and $G$ are linear combinations of increasing functions in $\NBV$ by \cref{thm:3.27}. By a simple calculation, it suffices to assume $F$ and $G$ are increasing. \dhcomment{Come back.} Suppose w.l.o.g. that $G$ is continuous, and let $\Omega = \{(x, y) : a < x \leq y \leq b\}$. We use Fubini's to compute $\mu_F \times \mu_G(\Omega)$ in two ways. \begin{align*}
