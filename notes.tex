...
To start , suppose $A \times B$ is a rectangle that is a (finite or countable) disjoint union of rectangles $A_j \times B_j$. Then for $x \in X$ and $y \in Y$, \[\chi_A(x) \chi_B(y) = \chi_{A \times B} (x, y) = \sum \chi_{A_j \times B_j}(x, y) = \sum \chi_{A_j} (x) \chi_{B_j}(y).\] Integrating with respect to $x$, adopting the usual convention that $0 \cdot \infty = 0$, and recalling \cref{thm:2.15}, we have \begin{align*}
